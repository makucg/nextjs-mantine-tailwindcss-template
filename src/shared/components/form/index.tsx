...
import { useForm, UseFormRegister, UseFormReturn, FieldValues } from 'react-hook-form';
...
const Form = (props: FormProps<any>) => {
	...
	const { children, methods = defaultMethods, onSubmit = () => {}, ...formProps } = props;
...
Form.TextInput = function TextInput(props: InputProps & TextInputProps) {
	...
	return <MantineTextInput {...(register && { ...register(name) })} {...textInputProps} />;
};

Form.PasswordInput = function PasswordInput(props: InputProps & PasswordInputProps) {
	const { name, register, ...passwordInputProps } = props;

	return <MantinePasswordInput {...(register && { ...register(name) })} {...passwordInputProps} />;
};
