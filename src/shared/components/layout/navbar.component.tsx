...
const useStyles = createStyles(theme => {
	...
	const orange = theme.colorScheme === 'dark' ? theme.colors.orange[7] : theme.colors.orange[5];
	return {
		logo: {
			position: 'relative',
			color: orange,
			...
			<Container className={classes.container} size={props.width}>
				<nav>
					<NextLink href='/' passHref>
						<Anchor className={classes.logo} underline={false}>
							nextjs-mantine-tailwindcss-template
						</Anchor>
					</NextLink>
					<ul className={classes.list}>
						{props.links?.map((link, i) => {
							const key = `link-${i}`;
							const displayNav =
								link.displayNav instanceof Function ? link.displayNav({ router, session }) : link.displayNav;
							if (!displayNav) return <React.Fragment key={key}></React.Fragment>;
							return (
								<li key={key} className={classes.listItem}>
									<NextLink href={link.path} passHref>
										<Anchor className={router.pathname === link.path ? 'active' : ''} underline={false}>
											{link.label}
										</Anchor>
									</NextLink>
								</li>
				...
				<Group>
					...
					{session.status === 'authenticated' && (
						<ActionIcon onClick={handleLogout} variant='filled' radius='xl' size='lg' color='orange'>
